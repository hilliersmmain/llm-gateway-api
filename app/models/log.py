...
class GuardrailLog(SQLModel, table=True):
    ...
    id: int | None = Field(default=None, primary_key=True)
    input_prompt: str = Field(..., sa_type=Text, description="The blocked input message")
    blocked_keyword: str | None = Field(
        default=None, sa_type=Text, description="The keyword that triggered the block"
    )
    ...
    client_ip: str | None = Field(default=None, sa_type=Text, description="Client IP address")
